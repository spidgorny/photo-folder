...
if (!JWT_SECRET) {
  throw new Error('JWT_SECRET or IRON_PASSWORD environment variable is required');
}
...
export interface JWTPayload {
  userId: string;
  email: string;
  allowedPrefixes?: string[];  // now optional
  role?: 'user' | 'admin';
...
// Helper to get token from Authorization header (mobile) or cookie (web)
...
export function isValidUser(email: string): boolean {
  const validUsers = process.env.VALID_USERS ? JSON.parse(process.env.VALID_USERS) : [];
